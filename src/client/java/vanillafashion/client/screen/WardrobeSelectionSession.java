...
import vanillafashion.client.fashion.FullFashionDraft;
...
public final class WardrobeSelectionSession {
	private vanillafashion.client.fashion.FullFashionDraft fullDraft;
    private boolean v2;
    private boolean persistenceReadOnly;
    private Optional<vanillafashion.fashion.FullFashionSelectionStatus> fullError = Optional.empty();
    public void observeFull(Optional<vanillafashion.fashion.FullPlayerFashionState> current, ClientPlayerFashionRegistry.State state) {
        ...
        if (fullDraft==null) fullDraft=new vanillafashion.client.fashion.FullFashionDraft(current.orElseThrow());
    ...
    public Optional<vanillafashion.network.SetFullFashionSelectionPayload> finishFull(
            vanillafashion.client.network.ClientFullFashionRequestTracker tracker, boolean channelSupported, Predicate<CapeId> metadataPresent) {
        return finishFull(tracker,channelSupported,true,metadataPresent,(part,id) -> false);
    }
    public boolean acceptFullResult(vanillafashion.network.FullFashionSelectionResultPayload result,
            Optional<vanillafashion.fashion.FullPlayerFashionState> latest, ClientPlayerFashionRegistry.State state) {
        ...
        boolean success=result.status()==vanillafashion.fashion.FullFashionSelectionStatus.SUCCESS;
    ...
    public Optional<vanillafashion.network.SetFullFashionSelectionPayload> finishFull(
            vanillafashion.client.network.ClientFullFashionRequestTracker tracker, boolean canSend, boolean receiverReady,
            Predicate<CapeId> capeAdmitted, BiPredicate<OutfitPart,OutfitId> outfitAdmitted) {
        ...
        return Optional.of(new vanillafashion.network.SetFullFashionSelectionPayload(pendingRequestId,fullDraft.baseline().revision(),fullDraft.draft()));
    }
