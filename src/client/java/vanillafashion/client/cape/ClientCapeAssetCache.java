...
import vanillafashion.cape.CapePngValidator;
...
public final class ClientCapeAssetCache {
	...
	public Optional<byte[]> findValidated(String sha256) {
		...
		try {
			...
			logger.warn("Vanilla Fashion 读取 Cape 缓存失败，已按 cache miss 处理：{}。", shortHash(sha256));
			return Optional.empty();
		}
	}
	...
	public StoreResult storeValidated(String sha256, byte[] pngBytes) {
		...
		try {
			...
			logger.warn("Vanilla Fashion 写入 Cape 内容缓存失败：{}。", shortHash(sha256));
			return StoreResult.WRITE_FAILED;
		} finally {
			if (temporaryFile != null) {
				try {
					...
					logger.warn("Vanilla Fashion 无法删除 Cape 缓存临时文件：{}。", shortHash(sha256));
				}
			}
		}
	}
	...
	public boolean invalidate(String sha256) {
		...
		try {
			...
			logger.warn("Vanilla Fashion 无法删除损坏的 Cape 缓存：{}。", shortHash(sha256));
			return false;
		}
	}
