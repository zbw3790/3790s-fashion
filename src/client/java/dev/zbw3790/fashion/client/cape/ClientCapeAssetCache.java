 ...
 */
public final class ClientCapeAssetCache {
	private static final String CACHE_DIRECTORY_NAME = "3790s-fashion";
	...
	public ClientCapeAssetCache(Path assetsDirectory, Logger logger) {
		this.assetsDirectory = Objects.requireNonNull(
				assetsDirectory,
				"客户端 Cape 缓存目录不能为 null。"
		).toAbsolutePath().normalize();
		this.logger = Objects.requireNonNull(logger, "客户端 Cape 缓存日志记录器不能为 null。");
	}

	public static ClientCapeAssetCache fromGameDirectory(Path gameDirectory, Logger logger) {
	...
	public Path pathFor(String sha256) {
		String validatedHash = CapeAssetHash.requireValid(sha256, "Cape 缓存 SHA-256 ");
	...
	public Optional<byte[]> findValidated(String sha256) {
		...
		try {
			...
			if (size < 1 || size > CapeAssetLimits.MAX_ASSET_BYTES) {
				invalidateCorruptEntry(path, sha256);
				return Optional.empty();
			}
			...
			logger.warn("3790's Fashion 读取 Cape 缓存失败，已按 cache miss 处理：{}。", shortHash(sha256));
			return Optional.empty();
		}
	}

	public StoreResult storeValidated(String sha256, byte[] pngBytes) {
		CapeAssetHash.requireValid(sha256, "Cape 缓存 SHA-256 ");
		Objects.requireNonNull(pngBytes, "Cape 缓存 PNG 字节不能为 null。");
		...
		try {
			...
			logger.warn("3790's Fashion 写入 Cape 内容缓存失败：{}。", shortHash(sha256));
			return StoreResult.WRITE_FAILED;
		} finally {
			if (temporaryFile != null) {
				try {
					...
					logger.warn("3790's Fashion 无法删除 Cape 缓存临时文件：{}。", shortHash(sha256));
				}
			}
		}
	}
	...
	public boolean invalidate(String sha256) {
		...
		try {
			...
			logger.warn("3790's Fashion 无法删除损坏的 Cape 缓存：{}。", shortHash(sha256));
			return false;
		}
	}
	...
	private static boolean isValid(String sha256, byte[] bytes) {
		return bytes.length >= 1
				&& bytes.length <= CapeAssetLimits.MAX_ASSET_BYTES
				&& sha256.equals(CapeAssetHash.sha256(bytes))
				&& CapePngValidator.validate(bytes) == CapePngValidator.ValidationResult.VALID;
	}
