package dev.zbw3790.fashion.client.asset;
 ...
 */
public class ValidatedAssetCache {
    ...
    private final java.util.function.Predicate<byte[]> validator;

	public ValidatedAssetCache(Path assetsDirectory, Logger logger, int maximum, java.util.function.Predicate<byte[]> validator) {
		this.maximum=maximum;this.validator=Objects.requireNonNull(validator);
        this.assetsDirectory = Objects.requireNonNull(
				assetsDirectory,
				"客户端 资产 缓存目录不能为 null。"
		).toAbsolutePath().normalize();
		this.logger = Objects.requireNonNull(logger, "客户端 资产 缓存日志记录器不能为 null。");
	}
	...
	public Path pathFor(String sha256) {
		String validatedHash = CapeAssetHash.requireValid(sha256, "资产 缓存 SHA-256 ");
	...
	public Optional<byte[]> findValidated(String sha256) {
		...
		try {
			...
			logger.warn("3790's Fashion 读取 资产 缓存失败，已按 cache miss 处理：{}。", shortHash(sha256));
			return Optional.empty();
		}
	}

	public StoreResult storeValidated(String sha256, byte[] pngBytes) {
		CapeAssetHash.requireValid(sha256, "资产 缓存 SHA-256 ");
		Objects.requireNonNull(pngBytes, "资产 缓存 PNG 字节不能为 null。");
		...
		try {
			...
			logger.warn("3790's Fashion 写入 资产 内容缓存失败：{}。", shortHash(sha256));
			return StoreResult.WRITE_FAILED;
		} finally {
			if (temporaryFile != null) {
				try {
					...
					logger.warn("3790's Fashion 无法删除 资产 缓存临时文件：{}。", shortHash(sha256));
				}
			}
		}
	}
	...
	public boolean invalidate(String sha256) {
		...
		try {
			...
			logger.warn("3790's Fashion 无法删除损坏的 资产 缓存：{}。", shortHash(sha256));
			return false;
		}
	}
	...
	private boolean isValid(String sha256, byte[] bytes) {
		return bytes.length >= 1
				&& bytes.length <= maximum
				&& sha256.equals(CapeAssetHash.sha256(bytes))
				&& validator.test(bytes);
	}
