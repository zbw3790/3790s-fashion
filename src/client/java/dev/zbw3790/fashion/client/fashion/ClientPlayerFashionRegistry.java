...
public final class ClientPlayerFashionRegistry {
    ...
    public Optional<FullPlayerFashionState> fullAuthority(UUID id) { return route==FashionAuthorityRoute.V4?full.find(id):Optional.empty(); }
    public Optional<FullPlayerFashionState> fullSelfAuthority(UUID id) { return route==FashionAuthorityRoute.V4?full.selfAuthority(id):Optional.empty(); }
    ...
    public void receiveFullSnapshot(Object connection, FullPlayerFashionSnapshot snapshot) {
        ...
        route=FashionAuthorityRoute.V4; entries.clear(); resultConfirmedSelf=Optional.empty(); full.replace(connection,snapshot);
    }
    public ClientFullPlayerFashionRegistry.Acceptance receiveFullUpdate(Object connection, FullPlayerFashionEntry entry) {
        return route==FashionAuthorityRoute.V4?full.update(connection,entry):ClientFullPlayerFashionRegistry.Acceptance.IGNORED;
    }
    public ClientFullPlayerFashionRegistry.Acceptance receiveFullRemove(Object connection, FullPlayerFashionRemovePayload payload) {
        return route==FashionAuthorityRoute.V4?full.remove(connection,payload):ClientFullPlayerFashionRegistry.Acceptance.IGNORED;
    }
	...
	public void replace(PlayerFashionSnapshot snapshot) {
        if (route==FashionAuthorityRoute.V4) return; route=FashionAuthorityRoute.LEGACY; full.unsupported(connection);
	...
	public void update(PlayerFashionEntry entry) {
        if (route==FashionAuthorityRoute.V4) return;
	...
	public Optional<PlayerFashionAuthoritativeState> getKnownState(UUID playerId) {
        if (route==FashionAuthorityRoute.V4) return full.find(playerId).map(FullPlayerFashionState::capeProjection);
	...
	public State state() {
        if (route==FashionAuthorityRoute.V4) return switch(full.state()) { case KNOWN -> State.AVAILABLE; case UNAVAILABLE -> State.UNAVAILABLE; default -> State.UNINITIALIZED; };
        return state;
	}
	...
	public Optional<PlayerFashionAuthoritativeState> selfAuthority(UUID playerId) {
        if (route==FashionAuthorityRoute.V4) return getKnownState(playerId);
	...
	public int size() {
        return route==FashionAuthorityRoute.V4?full.size():entries.size();
	}
