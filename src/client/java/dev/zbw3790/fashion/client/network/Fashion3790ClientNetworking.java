...
import dev.zbw3790.fashion.network.PlayerFashionSnapshotPayload;
import dev.zbw3790.fashion.network.PlayerFashionUpdatePayload;
import dev.zbw3790.fashion.network.PlayerFashionRemovePayload;
...
import dev.zbw3790.fashion.network.CapeRegistrySnapshotPayload;
import dev.zbw3790.fashion.network.OpenWardrobePayload;
import dev.zbw3790.fashion.network.WardrobeAvailablePayload;
import dev.zbw3790.fashion.wardrobe.WardrobeServerAvailability;

public final class Fashion3790ClientNetworking {
	private Fashion3790ClientNetworking() {
	}
	...
	public static void register(
		...
		ClientPlayConnectionEvents.DISCONNECT.register((handler, client) -> {
			scheduleConnectionStateClear(client::execute, () -> {
				...
				logger.info("3790's Fashion 当前连接的玩家时装、衣柜可用性、Cape Registry、Asset Store、pending 与动态纹理状态已清除；磁盘内容缓存保持不变。");
			});
		});
		...
		boolean availabilityReceiverRegistered = registerCurrentConnectionReceiver(
				...
				(payload, context) -> {
					...
					logger.info("3790's Fashion 当前服务器已声明支持衣柜功能。");
				}
		);
		...
		boolean assetDataReceiverRegistered = registerCurrentConnectionReceiver(
				...
				(payload, context) -> {
					...
					if (result != ClientCapeAssetSync.ReceiveResult.STORED) {
						logger.warn(
								"3790's Fashion 已拒绝服务器发送的 Cape 资产：{}，原因={}。",
					...
					assetSync.lastCacheStoreResult().ifPresent(cacheResult -> {
						if (cacheResult == ClientCapeAssetCache.StoreResult.WRITE_FAILED) {
							logger.warn(
									"3790's Fashion Cape 资产已进入连接内存，但未能持久化到内容缓存：{}。",
									shortHash(payload.sha256())
							);
						}
					});
					...
					if (FabricLoader.getInstance().isDevelopmentEnvironment()) {
						logger.info(
								"3790's Fashion 已接收 Cape 资产：{}，{} bytes；Asset Store 当前 {} 项，纹理结果 {}。",
								shortHash(payload.sha256()),
		...
		boolean openReceiverRegistered = registerCurrentConnectionReceiver(
				...
				(payload, context) -> {
					...
					if (!availability.isAvailable()) {
						availability.markAvailable();
						logger.debug("3790's Fashion 已通过 OpenWardrobe 安全兜底建立衣柜可用性状态。");
					}
		...
		if (!availabilityReceiverRegistered
			...
			throw new IllegalStateException("3790's Fashion 客户端 payload 接收器重复注册。");
		}

		logger.info("3790's Fashion legacy Cape 接收器与衣柜发送桥接已注册。");
	}
	...
	private static void requestMissingAssets(
			dev.zbw3790.fashion.cape.CapeRegistrySnapshot snapshot,
			ClientCapeAssetStore assetStore,
			ClientCapeAssetSync assetSync,
			ClientCapeTextureManager capeTextureManager,
			TextureManager minecraftTextureManager,
		...
		if (FabricLoader.getInstance().isDevelopmentEnvironment()) {
			logger.info(
					"3790's Fashion 当前 Snapshot 引用 {} 个 unique 资产：缓存命中 {} 个，新注册纹理 {} 个，释放纹理 {} 个，需要请求 {} 个。",
		...
		for (CapeAssetRequestPayload request : requests) {
			...
			if (FabricLoader.getInstance().isDevelopmentEnvironment()) {
				logger.info(
						"3790's Fashion 已发送 Cape 资产 C2S 请求：{} 个 unique hash。",
	...
	private static void logSnapshot(ClientCapeRegistry capeRegistry, Logger logger) {
		if (!FabricLoader.getInstance().isDevelopmentEnvironment()) {
			logger.debug("3790's Fashion 已接收服务器 Cape Registry：{} 个条目。", capeRegistry.size());
			return;
		}

		logger.info("3790's Fashion 已接收服务器 Cape Registry：{} 个条目。", capeRegistry.size());

		for (CapeCosmeticMetadata metadata : capeRegistry.entries()) {
			logger.info(
					"3790's Fashion Cape 元数据：ID={}，cape SHA-256={}，elytra SHA-256={}。",
					metadata.id(),
					metadata.capeSha256(),
