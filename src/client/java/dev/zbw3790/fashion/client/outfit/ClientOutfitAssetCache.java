...
public final class ClientOutfitAssetCache {
	...
	public Optional<byte[]> findValidated(String sha256) {
		...
		try {
			...
			logger.warn("3790's Fashion 读取 Outfit 缓存失败，已按 cache miss 处理：{}。", shortHash(sha256));
			return Optional.empty();
		}
	}
	...
	public StoreResult storeValidated(String sha256, byte[] pngBytes) {
		...
		try {
			...
			logger.warn("3790's Fashion 写入 Outfit 内容缓存失败：{}。", shortHash(sha256));
			return StoreResult.WRITE_FAILED;
		} finally {
			if (temporaryFile != null) {
				try {
					...
					logger.warn("3790's Fashion 无法删除 Outfit 缓存临时文件：{}。", shortHash(sha256));
				}
			}
		}
	}
	...
	public boolean invalidate(String sha256) {
		...
		try {
			...
			logger.warn("3790's Fashion 无法删除损坏的 Outfit 缓存：{}。", shortHash(sha256));
			return false;
		}
	}
