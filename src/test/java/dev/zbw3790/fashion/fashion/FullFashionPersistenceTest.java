...
class FullFashionPersistenceTest {
    ...
    Path frozen(Path directory) throws IOException {
        ...
        try(var input=getClass().getResourceAsStream("/dev/zbw3790/fashion/fashion/frozen-v021-player-fashion.dat")){assertNotNull(input);Files.copy(input,file);}
    ...
    @Test void frozenV1LoadsWithoutDirtyOrRewriteThenActualMutationWritesCurrentSchema() throws Exception {
        ...
        var root=NbtIo.readCompressed(file,NbtAccounter.create(PlayerFashionPersistence.MAX_NBT_BYTES));assertEquals(4,root.getCompound("data").orElseThrow().getIntOr("schema_version",-1));
    ...
    @Test void independentProcessReadsWrittenCurrentSchema() throws Exception {
        String childFile=System.getProperty("fashion3790.s03RestartFile");
        ...
        String classpath=System.getProperty("fashion3790.testRuntimeClasspath");assertNotNull(classpath);
        Path arguments=temp.resolve("restart.args");String content="-cp\n"+quote(classpath)+"\ndev.zbw3790.fashion.fashion.S03PersistenceSubprocess\n"+quote(directory.toString())+"\n";Files.writeString(arguments,content);
    ...
    @Test void sparseDeterministicCurrentSchemaKeepsNoneAndDormantWithoutRegistryQueries() {
    ...
    void everyBadEntryProtectsEntireActualFileAndRejectsBothWriteRoutes(String damage) throws Exception {
        ...
        switch(damage){
            ...
            case "missing_id"->part.remove("outfit_id");case "none_with_id"->part.putString("mode","none");case "bad_id"->part.putString("outfit_id","../bad");case "cape_type"->entry.putInt("cape_id",1);case "bad_uuid"->entry.putString("uuid","1-1-1-1-1");case "version"->data.putInt("schema_version",5);case "truncated"->{}
    ...
    @ParameterizedTest @ValueSource(ints={0,15}) void maximum16384ShapePassesActual64MiBBoundedLoaderAndOrdinarySave(int armorMask) throws Exception {
        ...
        var state=new PlayerFashionStoredState(Optional.of(new CapeId(longest)),outfit,new dev.zbw3790.fashion.armor.ArmorSelections(armorMask));for(int i=0;i<16384;i++)data.setState(new UUID(0,i),state);
        ...
        var budget=NbtAccounter.create(PlayerFashionPersistence.MAX_NBT_BYTES);NbtIo.readCompressed(file,budget);if(armorMask==0)assertEquals(62882276,budget.getUsage());else assertTrue(budget.getUsage()<PlayerFashionPersistence.MAX_NBT_BYTES);assertEquals(67108864,PlayerFashionPersistence.MAX_NBT_BYTES);
