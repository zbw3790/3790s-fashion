...
class ReleaseArchitectureAuditTest {
	...
	void allServerSendsUseSingleCapabilityGate() throws IOException {
		...
		assertEquals(List.of(javaRoot.resolve("dev/zbw3790/fashion/network/ServerPayloadSender.java")), senders);
	...
	void releaseMetadataRemainsConsistent() throws IOException {
		...
		assertTrue(metadata.contains("fashion_3790"));
		assertTrue(metadata.contains("3790's Fashion"));
		assertTrue(metadata.contains("MIT"));
		assertTrue(metadata.contains("assets/fashion_3790/icon.png"));
    ...
    @Test void exactlyOneCommonInteractionMixinAndNoForwardCallback() throws IOException {
        ...
        var config=JsonParser.parseString(read(root().resolve("src/main/resources/fashion_3790.mixins.json"))).getAsJsonObject();
        ...
        assertTrue(read(root().resolve("src/main/resources/fabric.mod.json")).contains("fashion_3790.mixins.json"));
        String mixin=read(root().resolve("src/main/java/dev/zbw3790/fashion/mixin/PlayerInteractionMixin.java"));
    ...
    @Test void optionalVisualContractDeclaresTheIntegerCapability() {
        String source = read(root().resolve("src/main/resources/fabric.mod.json"));
        String processed = read(root().resolve("build/resources/main/fabric.mod.json"));
        for (String metadata : List.of(source, processed)) {
            var marker = JsonParser.parseString(metadata).getAsJsonObject().getAsJsonObject("custom")
                    .getAsJsonPrimitive("elytra_slot_3790:visual_contract");
            assertTrue(marker.isNumber());
            assertEquals("1", marker.getAsString(), "能力标记必须是整数词元 1，不能使用字符串、小数或指数。");
        }
    }
