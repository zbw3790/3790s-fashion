...
class CapeSelectionHandlerTest {
	...
	private CapeSelectionHandler.Outcome process(PlayerFashionService service, Optional<vanillafashion.cape.CapeId> choice) {
	...
	void noChangeAcknowledgesWithoutDirtyOrDelta(boolean custom) {
		var choice = custom ? Optional.of(FOUNDER) : Optional.<vanillafashion.cape.CapeId>empty();
	...
	@Test void rejectedCapeReturnsExistingEffectiveNotDraft() {
		...
		var outcome = process(service(data, valid(root)), Optional.of(new vanillafashion.cape.CapeId("missing")));
	...
	void unsafeServiceRejectsBothCustomAndVanilla(String failure) {
		...
		for (var choice : List.of(Optional.<vanillafashion.cape.CapeId>empty(), Optional.of(BUILDER))) {
	...
	@Test void unavailableCapeDoesNotInvokePermissionPolicy() {
		...
		assertRejected(process(service, Optional.of(new vanillafashion.cape.CapeId("absent"))),
	...
	@Test void storageLimitRejectsNewRecordButAllowsClearAndReplacement() {
		var records = new HashMap<UUID, vanillafashion.cape.CapeId>();
	...
	@Test void rejectedRequestReturnsDormantStoredAndEffectivePair() {
		...
		var outcome = process(service, Optional.of(new vanillafashion.cape.CapeId("missing")));
		assertRejected(outcome, CapeSelectionReason.CAPE_NOT_AVAILABLE,
	...
	void unchangedOrRejectedSendsOnlyReply(boolean rejected) {
		var outcome = process(service(data(Map.of()), valid(root)),
				rejected ? Optional.of(new vanillafashion.cape.CapeId("missing")) : Optional.empty());
