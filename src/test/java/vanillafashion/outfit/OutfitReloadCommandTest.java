...
class OutfitReloadCommandTest {
    static class Source extends CommandSourceStack {
        ...
        @Override public void sendSuccess(Supplier<Component> message,boolean notify){success.add(message.get().getString());}
    ...
    @Test void adminAndConsoleUseOfficialPermissionAndDelegateOnce() throws Exception {
        for(var source:List.of(new Source(LevelBasedPermissionSet.GAMEMASTER,"管理员"),new Source(PermissionSet.ALL_PERMISSIONS,"控制台"))) {
            ...
            assertEquals(1,dispatcher.execute("vanillafashion reload",source));assertEquals(1,calls.get());assertEquals(List.of("装束重载完成"),source.success);assertTrue(source.failure.isEmpty());
        }
    }
    @Test void unauthorizedPlayerAndModeratorCannotInvokeReload() {
        for(var permissions:List.of(PermissionSet.NO_PERMISSIONS,LevelBasedPermissionSet.MODERATOR)) {
            ...
            assertThrows(CommandSyntaxException.class,()->dispatcher.execute("vanillafashion reload",new Source(permissions,"普通玩家")));
        }
    }
    @Test void serviceFailureUsesFailureChannelAndReturnsZero() throws Exception {
        ...
        assertEquals(0,dispatcher.execute("vanillafashion reload",source));assertEquals(List.of("根目录不可信"),source.failure);assertTrue(source.success.isEmpty());
    }
    @Test void noChangeStillReportsSuccessWithoutInventingRefreshCount() throws Exception {
        ...
        assertEquals(1,dispatcher.execute("vanillafashion reload",source));assertEquals(List.of("内容没有变化"),source.success);assertTrue(source.failure.isEmpty());
    }
}
