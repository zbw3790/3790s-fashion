...
import vanillafashion.cape.CapeAssetIndex;
import vanillafashion.cape.CapeAssetReadResult;
import vanillafashion.cape.CapeAssetReader;
...
import vanillafashion.cape.CapeTextureAsset;

public final class VanillaFashionNetworking {
	private VanillaFashionNetworking() {
	}
	...
	public static void register(CapeRegistryService capeRegistryService, Logger logger) {
		...
		ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> networking.execute(server, channel -> {
			...
			if (registry.size() > CapeRegistrySnapshot.MAX_CAPE_ENTRIES) {
				logger.error(
						"Vanilla Fashion Cape Registry 含有 {} 个条目，超过网络上限 {}；未向玩家发送 Snapshot。",
						registry.size(),
			...
			try {
				...
				if (FabricLoader.getInstance().isDevelopmentEnvironment()) {
					logger.info(
							"Vanilla Fashion 已向玩家发送 Cape Registry Snapshot：{} 个条目。",
							snapshot.size()
					);
				}
	...
	private static void handleAssetRequest(
		...
		if (FabricLoader.getInstance().isDevelopmentEnvironment()) {
			logger.info(
					"Vanilla Fashion 已收到 Cape 资产请求：{} 个 unique hash，当前 AssetIndex {} 项。",
		...
		for (String hash : payload.sha256Hashes()) {
			...
			if (claim == CapeAssetRequestTracker.ClaimResult.LIMIT_REACHED) {
				logger.warn("Vanilla Fashion 已忽略玩家当前连接超出 2048 个 hash 的后续 Cape 资产请求。");
				return;
			}

			if (claim == CapeAssetRequestTracker.ClaimResult.DUPLICATE) {
				logger.debug("Vanilla Fashion 已忽略当前连接重复请求的 Cape 资产：{}。", shortHash(hash));
				continue;
			}
			...
			if (asset == null) {
				logger.debug("Vanilla Fashion 已忽略当前 Registry 中不存在的 Cape 资产请求：{}。", shortHash(hash));
				continue;
			}
			...
			if (readResult.status() == CapeAssetReadResult.Status.SUCCESS) {
				...
				if (FabricLoader.getInstance().isDevelopmentEnvironment()) {
					logger.info(
							"Vanilla Fashion 已发送 Cape 资产：{}，{} bytes。",
							shortHash(hash),
							bytes.length
					);
				}
			...
			if (readResult.status() == CapeAssetReadResult.Status.HASH_MISMATCH) {
				logger.warn(
						"Vanilla Fashion Cape 资产在 Registry 加载后发生变化，需要重启或重新加载服务器后才能同步：{}。",
						shortHash(hash)
				);
			} else {
				logger.warn(
						"Vanilla Fashion Cape 资产发送前读取校验失败，本次未发送：{}，状态={}。",
						shortHash(hash),
						readResult.status()
				);
			}
