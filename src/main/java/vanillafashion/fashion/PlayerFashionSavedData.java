...
public final class PlayerFashionSavedData extends SavedData {
    ...
    public static final SavedDataType<PlayerFashionSavedData> TYPE = new SavedDataType<>(Identifier.fromNamespaceAndPath("vanilla_fashion", "player_fashion"), PlayerFashionSavedData::new, CODEC, null);
    ...
    private static DataResult<PlayerFashionSavedData> decode(CompoundTag tag) {
        try {
            ...
            if (version != 1 && version != 2) throw new IllegalArgumentException("存档版本不受支持。");
            ...
            for (Tag value : entries) {
                ...
                fields(entry, version==1 ? Set.of("uuid", "cape_id") : Set.of("uuid", "cape_id", "outfit_parts"));
                ...
                var state=new PlayerFashionStoredState(cape,outfit); if (!state.isDefault()) data.states.put(id,state);
            }
    ...
    private CompoundTag encode() {
        ...
        states.entrySet().stream().sorted(Map.Entry.comparingByKey()).forEach(item -> {
            ...
            if (!parts.isEmpty()) entry.put("outfit_parts",parts);entries.add(entry);
        });
