...
public final class CapeRegistryLifecycle {
	...
	public static CapeRegistryKnowledge load(CapeRegistryService service, Path capesRoot, Logger logger) {
		logger.info("3790's Fashion Cape Registry 开始加载，资产目录：{}。", capesRoot.toAbsolutePath().normalize());
		...
		try {
			...
			logger.info(
					"3790's Fashion Cape Registry 加载完成：{} 个有效，{} 个无效。",
			...
			service.clear();
			logger.error(
					"3790's Fashion Cape Registry 加载失败，当前使用空 Registry。目录：{}。",
