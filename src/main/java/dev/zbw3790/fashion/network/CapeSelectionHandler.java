...
public final class CapeSelectionHandler {
	...
	public static Optional<Outcome> process(UUID sender, SetCapeSelectionPayload request,
        ...
        return process(sender, request, service, dev.zbw3790.fashion.fashion.FashionAuthorityRoute.LEGACY, resultSupported, synchronizationAvailable);
    }
    public static Optional<Outcome> process(UUID sender, SetCapeSelectionPayload request,
            Optional<PlayerFashionService> service, dev.zbw3790.fashion.fashion.FashionAuthorityRoute route, boolean resultSupported, boolean synchronizationAvailable) {
        if (route!=dev.zbw3790.fashion.fashion.FashionAuthorityRoute.LEGACY) return Optional.empty();
        if (!resultSupported) {
