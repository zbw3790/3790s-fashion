...
import dev.zbw3790.fashion.identity.LegacyIdentityMigration;
...
public final class PlayerFashionPersistence {
	...
	public static LoadResult load(SavedDataStorage storage, Path dataDirectory, Logger logger) {
        Path canonical = dataFile(dataDirectory);
        Path file = canonical;
        try {
            LegacyIdentityMigration.requireSafePath(canonical);
            if (Files.notExists(canonical, LinkOption.NOFOLLOW_LINKS)) {
                Path legacy = dataDirectory.resolve(LegacyIdentityMigration.LEGACY_NAMESPACE).resolve("player_fashion.dat");
                LegacyIdentityMigration.requireSafePath(legacy);
                if (!Files.notExists(legacy, LinkOption.NOFOLLOW_LINKS)) file = legacy;
            }
			boolean missing;
			try {
            ...
            if (!missing) {
                ...
                byte[] source;
                try (var bytes = Files.newInputStream(file)) {
                    source = bytes.readNBytes((int) MAX_NBT_BYTES + 1);
                    if (source.length > MAX_NBT_BYTES) throw new IOException("时装存档压缩输入超过读取上限。");
                }
                try (var input = new java.io.PushbackInputStream(new java.io.ByteArrayInputStream(source), 2)) {
                ...
                if (!file.equals(canonical)) {
                    LegacyIdentityMigration.publishValidatedFile(canonical, source);
                    logger.info("3790's Fashion 旧身份存档已验证并复制到新位置；保留旧文件，仅注册一份权威状态。");
                }
	...
	private static LoadResult degraded(Logger logger, String reason) {
		logger.warn("3790's Fashion 持久化已降级为只读：{}", reason);
